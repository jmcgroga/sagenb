...
class NotebookRunPassenger(NotebookRun):
    name="passenger"
    PASSENGER_PREFIX_CONFIG = """
####################################################################
# WARNING -- Do not edit this file!   It is autogenerated each time
# the notebook(...) command is executed.
####################################################################
...
os.environ['SAGE_SHARE'] = SAGE_SHARE
"""
    PASSENGER_POST_CONFIG = """
...
"""
    ...
    def run_command(self, kw):
        ...
        self.prepare_kwds(kw)
        kw['SAGE_ROOT'] = SAGE_ROOT
        run_file = os.path.join(kw['directory'], 'passenger_wsgi.py')

        with open(run_file, 'w') as script:
            script.write((self.PASSENGER_PREFIX_CONFIG+self.config_stub+self.PASSENGER_POST_CONFIG)%kw)
        print "Passenger configuration file written to '%s'" % (run_file)
        cmd = 'passenger start --app-type wsgi --startup-file %s' % (run_file)
        return cmd
...
command={'flask': NotebookRunFlask, 'twistd': NotebookRunTwisted, 'uwsgi': NotebookRunuWSGI, 'tornado': NotebookRunTornado, 'passenger' : NotebookRunPassenger}
